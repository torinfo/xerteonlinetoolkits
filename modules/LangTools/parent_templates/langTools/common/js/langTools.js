...
function setUpInterface() {
	...
	$backBtn
		.click(function() {
			if ($introHolder.is(":visible") || $overviewHolder.is(":visible")) {
				...
				if (currentStep + 1 >= langToolsHistory[currentLangTools].length) {
					$fwdBtn.attr("disabled","disabled");
				}
			} else {
				currentStep--;
				setUpStep(langToolsHistory[currentLangTools][currentStep].id)
			}
			$dialog.dialog("close");
		})
	...
	$submitBtn
		...
		.click(function() {
			if (currentStep + 1 != langToolsHistory[currentLangTools].length) {
				langToolsHistory[currentLangTools] = langToolsHistory[currentLangTools].slice(0, currentStep + 1);
				storedResultTxt[currentLangTools] = storedResultTxt[currentLangTools].slice(0, currentStep + 1);
			}
			...
			if (currentStepInfo.type == "mcq") {
				...
				if (currentStepInfo.format == "menu") {
					...
					storedResultTxt[currentLangTools].push($step.find("select #opt" + answer).data("resultTxt"));
					...
					// store resultTxt (optional) so it can be shown as collated result at the end
					storedResultTxt[currentLangTools].push($step.find("input:checked").data("resultTxt"));
				}

				langToolsHistory[currentLangTools][langToolsHistory[currentLangTools].length-1].option = answer;
				...
				langToolsHistory[currentLangTools][langToolsHistory[currentLangTools].length-1].option = $stepHolder.find("#amount").val();
				...
				currentStepInfo.options.each(function(i) {
					...
					if ($slider.slider("value") >= $this.attr("min") && $slider.slider("value") <= $this.attr("max")) {
						...
						storedResultTxt[currentLangTools].push($this.attr("resultTxt"));
				...
				storedResultTxt[currentLangTools].push("");
	...
	if (allQParams.email != "false" && allQParams.email != "") {
		$("#emailBtn")
			.click(function() {
				...
				for (var i=0; i<langToolsHistory.length; i++) {
	...
	if (urlParams.step != undefined) {
		startNewLangTools(urlParams.step);
	} else {
		startNewLangTools();
	}
}
...
// _____ START NEW LANGTOOLS FROM 1st STEP _____
function startNewLangTools(urlStep) {
	if (allSteps.length == 0) {
		...
		// is it the 1st langTools?
		if (currentLangTools == undefined) {
			currentLangTools = 0;
		} else {
			currentLangTools++;
		}
		...
		currentStep = 0;
		langToolsHistory.push([]); // new array to hold history of steps for this langTools
...
function setUpStep(stepID, isNew) {
	...
	if (currentStepInfo != null) {
		...
		if (isNew) { // new step (not part of history)
			if (langToolsHistory[currentLangTools].length > 0) {
				currentStep++;
			} else {
				// info only gets added to this array when submit button clicked so add blank first entry so it keeps same length as langToolsHistory
				storedResultTxt[currentLangTools].push("");
			}
			langToolsHistory[currentLangTools].push({id: currentStepInfo.name});
		}
		...
		if (currentStep + 1 >= langToolsHistory[currentLangTools].length && $introHolder.is(":hidden")) {
			$fwdBtn.attr("disabled", "disabled");
		} else {
			$fwdBtn.removeAttr("disabled");
		}
...
function setUpQ(isNew) {
	...
	if (currentStepInfo.built != false) {
		...
		// if part of history make sure it's showing the correct answer
			...
			if (currentStepInfo.type == "mcq") {
				var answer = langToolsHistory[currentLangTools][currentStep].option;
				if (answer == undefined) {
					answer = -1;
					$submitBtn.button("disable");
				} else {
				...
				if (currentStep + 1 >= langToolsHistory[currentLangTools].length) {
					...
					$slider.slider({value: Number(langToolsHistory[currentLangTools][currentStep].option)});
					$thisStep.find("#labelHolder #amount").val(langToolsHistory[currentLangTools][currentStep].option);
				}
...
function setUpI() {
	if (currentStepInfo.built != false && currentStepInfo.collate != "true") {
		...
		$stepHolder.prepend('<div class="step"><span class="fa ' + icon + ' fa-2x pull-left fa-border fa-fw"/><div class="info">' + mediaInfo[0] + authorSupport + addLineBreaks(iFrameCheck(currentStepInfo.text)) + collateResult(currentStepInfo.collate, currentLangTools, "html") + '</div></div>');
...
// _____ CALLED WHEN VIEW THIS LANGTOOLS BTN CLICKED _____
function viewThisClickFunct() {
	...
	if (currentStepInfo.type == "info" && currentStep < langToolsHistory[currentLangTools].length - 1) {
		langToolsHistory[currentLangTools] = langToolsHistory[currentLangTools].splice(0,currentStep + 1);
		storedResultTxt[currentLangTools] = storedResultTxt[currentLangTools].splice(0,currentStep + 1);
		$fwdBtn.attr("disabled", "disabled");
	}

	$overviewHolder.find(".langToolsInfo").parent().remove();
	$("#viewAllBtn").remove();
	showHideHolders($overviewHolder);

	showLangTools(currentLangTools);

	if (langToolsHistory.length > 1) {
	...
	// _____ VIEW ALL LANGTOOLSS BTN _____
	$("#viewAllBtn")
		.click(function() {
			$overviewHolder.find(".langToolsInfo").parent().remove();
			$("#viewAllBtn").remove();
...
// _____ DISPLAY STEPS TAKEN IN LANGTOOLS _____
function showLangTools(dec) {
...
function createDecStr(dec, type) {
	...
	if (type == "html") {
		string += '<div class="langToolsInfo"><h3>' + allParams.overviewString + " " + (dec + 1) + ':</h3>';
	} else {
		string += allParams.overviewString + " " + (dec + 1) + ':\n';
	}

	// add details of each step in langTools to string
	for (var i=0; i<langToolsHistory[dec].length; i++) {
		var thisStep = findStep(langToolsHistory[dec][i].id),
			media = "";
		...
		if (thisStep.type == "mcq" || thisStep.type == "slider") {
			...
			if (thisStep.type == "mcq") {
				if (type == "html") {
					...
					string += '<div class="overviewAnswer"><span class="fa fa-angle-right fa-fw"/>' + addLineBreaks($(thisStep.options[langToolsHistory[dec][i].option]).attr("name")) + '</div>';
					string += '<div class="extraInfo"><span class="fa fa-angle-right fa-fw"/>' + allParams.posAnswerString + ': ';

					thisStep.options.each(function(j) {
						if (j != langToolsHistory[dec][i].option) {
							string += addLineBreaks($(thisStep.options[j]).attr("name"));

							if (j+1 == thisStep.options.length || (j+1 == langToolsHistory[dec][i].option && j+2 == thisStep.options.length)) {
							} else {
								string += ', ';
							}
						}
					...
					string += '\n > ' + $(thisStep.options[langToolsHistory[dec][i].option]).attr("name");
				}

			} else if (thisStep.type == "slider") {
				var	answer = langToolsHistory[dec][i].option + ' ' + thisStep.unit,
					posAnswer = thisStep.min + ' - ' + thisStep.max + ' ' + thisStep.unit;

				if (thisStep.unitPos == "start") {
					answer = thisStep.unit + ' ' + langToolsHistory[dec][i].option;
	...
	if (type != "html") {
		...
		if (dec + 1 < langToolsHistory.length) {
			string += '-------------------------------------------------\n\n';
		}
	}
...
function jumpToStep(stepID) {
	if (currentStepInfo.type == "result" || (currentStepInfo.type == "info" && currentStepInfo.lastStep == "true")) {
		startNewLangTools(stepID);
	} else {
		// remove partially completed workflow from history
		if (currentLangTools == 0) {
			currentLangTools = undefined;
		} else {
			currentLangTools--;
		}
		langToolsHistory = langToolsHistory.splice(0,langToolsHistory.length - 1);
