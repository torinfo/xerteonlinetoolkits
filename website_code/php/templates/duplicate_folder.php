...
if(is_numeric($_POST['folder_id'])){
    if (has_rights_to_this_folder($_POST['folder_id'], $_SESSION['toolkits_logon_id'])) {
        ...
        copy_folder($folder_id, $parentfolder_id, $folder_name);
    }

}

function copy_folder($folder_id, $parentfolder_id, $folder_name){
...
;                   duplicate_template($new_template_id, $template);
        ...
        foreach ($folders as $folder){
            copy_folder($folder['folder_id'], $new_folder_id, $folder['folder_name']);
        }
    }
}

function duplicate_template($new_template_id, $template){
    switch($template['template_framework']){
        case 'xerte':
            duplicate_template_xerte($new_template_id, $template['template_id'], $template['org_template_name']);
            break;
        case 'site':
            duplicate_template_site($new_template_id, $template['template_id'], $template['org_template_name']);
            break;
        case 'decision':
            duplicate_template_decision($new_template_id, $template['template_id'], $template['org_template_name']);
            break;
        default:
            break;
    }
